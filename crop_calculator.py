...
クロップ方針は --mode で選ぶ:
  ...
  promo           宣材写真用の従来ロジック(固定ルール)。
                  ...
                  - 全身写真でない場合: 上部の余分な余白を削除しつつ、上部余白を約10%確保
                  - 全身写真の場合: 人物が切れないよう上下に約10%の余白を確保
...
# 調整可能パラメータ
# ----------------------------------------------------------------------
MARGIN_RATIO = 0.10          # 目標とする余白の割合(上下左右共通のベース値)
...
FOOT_PAD_RATIO = 0.03        # 足元(全身時)にわずかに足す余白(人物身長に対する比率)
                             # 手動クロップ実績(全身37枚)との比較で最適化した値。
                             # 手動では足元が一貫して広め(+0.012、標準偏差0.007)だった。
...
# --- 複数人が写っている写真への対応 ---
# 検出する人数の上限。複数人を検出した場合は全員を収めるようバウンディングボックスを
# 統合する(1人だけの写真では統合しても結果は変わらないため、影響はない)。
MAX_PERSONS = 3
# 背景に小さく写り込んだ人物まで含めると構図が破綻するため、最も大きい人物に対して
# 高さがこの比率未満の人物は除外する。
...
class PoseDetector:
    ...
    detect_pose() は (検出した全員分のランドマークのリスト, 段階のインデックス) を返す。
    リストの先頭が最も信頼度の高い人物。段階0は従来と同じ条件での検出を意味する。
    """
    ...
    def detect_pose(self, disp_img):
        ...
        for stage_index, (max_dim, confidence, contrast) in enumerate(stages):
            ...
            if result.pose_landmarks:
                # 妥当性チェックは最も信頼度の高い1人目に対して行う。
                # 段階0(従来条件)の結果は、これまでの動作を変えないため無条件に採用する。
                if stage_index == 0 or _is_plausible_pose(result.pose_landmarks[0]):
                    return list(result.pose_landmarks), stage_index
        return None, None
...
def analyze_image(path, landmarker):
    ...
    poses, detect_stage = landmarker.detect_pose(disp_img)
    if not poses:
        return None
...
def compute_crop(info):
    ...
    if info["full_body"]:
        # 上下とも約10%の余白を確保する必要がある高さ
        hc_from_height = person_h / (1 - 2 * MARGIN_RATIO)
    else:
        # 上部のみ約10%の余白を確保しつつ、人物の見えている範囲(y1〜y2)を
