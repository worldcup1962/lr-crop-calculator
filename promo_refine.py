...
【特徴量について: 試して不採用にしたもの】
特徴量を増やす方向は一度検証し、いずれも効果がなかったため採用していない
(手動クロップ789枚、ブロック単位5分割交差検証):

  画像由来(エッジ密度5次元 + セグメンテーションマスク6次元)  +0.0020
  ポーズ由来(腕の広がり・胴体の傾き・座り姿勢など11次元)      +0.0021

789枚でのIoUの標準誤差は約0.0036で、いずれも誤差の範囲内。しかも両方とも
複数人写真では悪化した(0.8623 -> 0.8527 / 0.8392)。次元を増やすと枚数の
少ない区分が過学習するため。

特にマスク特徴量は、モデルの重要度では上位に来る(mask_below が全体2位)のに
未知データでの精度は上がらなかった。学習データ内では説明力があっても汎化しない
典型で、木を増やしても頭打ちだった。

一方、学習曲線はまだ頭打ちになっていない:
  157枚 0.9259 / 315枚 0.9304 / 473枚 0.9344 / 631枚 0.9367
